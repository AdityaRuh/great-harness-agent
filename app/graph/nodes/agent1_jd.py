...
All nodes are async and use asyncio.to_thread() to run blocking LLM calls
without freezing the server event loop.
"""
...
async def agent1_analyze_repos(state: PipelineState) -> dict:
    logger.info(f"[Agent 1 / SA-1] Analyzing repos for: {state['role_title']}")
    profile = await asyncio.to_thread(analyze_tech_stack, state["role_title"], state["experience_level"])
    return {
...
async def agent1_map_skills(state: PipelineState) -> dict:
    logger.info("[Agent 1 / SA-2] Mapping skills")
    matrix = await asyncio.to_thread(map_skills, state["tech_stack_profile"], state["role_title"], state["experience_level"])
    return {
...
async def agent1_draft_jd(state: PipelineState) -> dict:
    ...
    return {
        ...
        "audit_log": [{
            "agent": "agent_1",
            "action": "jd_drafted",
            "explanation": f"Generated {len(jd.split())} word JD for {state['role_title']}. Awaiting HR approval.",
            "data": {"word_count": len(jd.split())},
        }],
    }


async def agent1_serialize_and_publish(state: PipelineState) -> dict:
